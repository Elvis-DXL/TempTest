...
import static com.elvis.test.base.base.DSUtil.EmptyTool.isEmpty;
import static com.elvis.test.base.base.DSUtil.EmptyTool.isNotEmpty;
...
public final class DSPo {
    ...
    public final static class TreeNode implements Serializable {
        ...
        public static List<TreeNode> formatTree(List<TreeNode> treeNodeList) {
            if (DSUtil.EmptyTool.isEmpty(treeNodeList)) {
                return new ArrayList<>();
            }
            List<TreeNode> result = new ArrayList<>();
            Map<String, TreeNode> tmpMap = DSUtil.ListTool.listToMap(treeNodeList, TreeNode::getSelfId, it -> it);
            List<TreeNode> childList;
            for (TreeNode item : treeNodeList) {
                if (DSUtil.EmptyTool.isNotEmpty(item.getParentId()) && null != DSUtil.mapGet(tmpMap, item.getParentId())
                        && !item.getSelfId().equals(item.getParentId())) {
                    childList = null == DSUtil.mapGet(tmpMap, item.getParentId()).getChildList() ?
                            new ArrayList<>() : DSUtil.mapGet(tmpMap, item.getParentId()).getChildList();
                    childList.add(item);
                    DSUtil.mapGet(tmpMap, item.getParentId()).setChildList(childList);
                } else {
                    result.add(item);
                }
            }
